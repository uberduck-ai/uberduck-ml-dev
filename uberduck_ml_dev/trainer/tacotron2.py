...
class Tacotron2Trainer(TTSTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.has_audio_encoder = (
            self.hparams.audio_encoder_path is not None
        )  # NOTE (Sam): this was autogenerated but maybe is preferable
        # NOTE (Sam): this needs to be generated ahead of time so in the future we should use this instead of the audio encoder to remove need to forward pass.
        ...
        # TODO (Sam): this should be done in the model itself since it acts on the xs.  This will remove redundancy.
        ...
        # NOTE (Sam): I thought that in contrast to the torchmoji encoder and the speaker encoder, the audio encoder belongs in the trainer since it acts only on the ys
        # However, teacher forcing does the same thing in a sense, and in general, trainer should aim to be model agnostic.
        # Let's think about it...

        # TODO (Sam): this makes sample inference work but really we should be using centroids for now.
    ...
    def sample_inference(self, model, transcription=None, speaker_id=None):
        ...
        with torch.no_grad():
            ...
            # NOTE (Sam): combine contextual encoders
            if self.compute_gst:
                gst_embedding = torch.FloatTensor(self.compute_gst([transcription]))
            else:
                gst_embedding = None
            # NOTE (Sam): audio_encoder is logically like teacher forcing
            if self.audio_encoder_forward:
                # NOTE (Sam): speaker_embedding here is slightly logically inconsistent with the one-hot version.
                # TODO (Sam): make this not sus.
